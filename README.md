...
| | |
|---|---|
| 代码 / 测试 | 约 13k 行 · **384 个测试全过**（`python -m pytest -q`；含 Node 跑的页面冒烟测试） |
...
| [CHANGE_RISK_PLAN.md](CHANGE_RISK_PLAN.md) | **改动与风险一页规格**（当前优先级：它改了什么 / 有没有乱改 / 在变好吗） | 🟡 S1 ✅ · S2 风险规则已交付（0.14.0），待你验收 → S3 |
...
tokmon/
  ...
  serve                        驾驶舱外壳
tests/                         384 例: pytest -q (tests/js/workflow_smoke.js: 页面 JS 的 Node 冒烟脚手架)
```
