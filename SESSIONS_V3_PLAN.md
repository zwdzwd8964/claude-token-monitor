...
> **【状态章 · 2026-09-20 核对】❌ 未实施（1164 行设计，0 行实现）。**
> 0.9.0 只取了其中**最小的诚实切片**：默认不再隐藏「等你」的会话 + 把语义反了的「空闲」改名「等你已久」。
> 其余（表格视图 / 分面计数 / `last_reply_epoch` / F7–F12）**仍未做**，要做请按 §10 的边界来。
> ⚠️ 文中「`MC_REMOTE` 尚未实现、看板读页裸奔」的警告**已于 0.9.0 解除**。
